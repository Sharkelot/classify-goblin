"""Goblin JEV command-line entry points.

The primary end-user path is:

    goblin-jev download-checkpoint --manifest <manifest.json> --cache-dir <dir>

The legacy ``jev-laya-free`` command remains available (see pyproject
``[project.scripts]``); this module adds the Goblin JEV branding/command
without changing the existing server entry point.
"""
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        prog="goblin-jev",
        description="Goblin JEV runtime: download and install the inference checkpoint.",
    )
    ...
    dl.add_argument("--cache-dir", default="~/.cache/goblin-jev", help="local checkpoint cache dir")

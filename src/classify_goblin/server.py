...
def main():
    ...
    if args.backend == 'laya':
        ...
        # (CLASSIFY_GOBLIN_QWEN_API_KEY) and never printed.
        endpoint = get_env('CLASSIFY_GOBLIN_QWEN_BASE_URL', 'http://127.0.0.1:8080')
        timeout = int(get_env('CLASSIFY_GOBLIN_QWEN_TIMEOUT', '30'))
        backend = QwenArtifactBackend(client=OpenAICompatClient(
            base_url=endpoint, timeout=timeout))
    else:
        backend = RulesBackend()
    server = LocalServer((args.host, args.port), backend, get_env('CLASSIFY_GOBLIN_LOCAL_API_KEY'))

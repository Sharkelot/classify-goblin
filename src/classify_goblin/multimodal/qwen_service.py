"""CG-MM-12: local Qwen typed-decision backend over an OpenAI-compatible endpoint.
...
* the endpoint origin must be loopback-only (``http://127.0.0.1[:port]`` or
  ...
  explicit (``base_url``) or resolved from ``CLASSIFY_GOBLIN_QWEN_BASE_URL``;
* the credential is resolved from ``CLASSIFY_GOBLIN_QWEN_API_KEY`` (env) and never embedded
...
modality (per the CG-MM-01 probe manifest) raises before any HTTP call; a
...
class OpenAICompatClient:
    ...
    def __init__(self, base_url=None, api_key=None, model=None, timeout=30, transport=None):
        if base_url is None:
            base_url = get_env('CLASSIFY_GOBLIN_QWEN_BASE_URL', 'http://127.0.0.1:8080')
        ...
        self.api_key = api_key if api_key is not None else get_env('CLASSIFY_GOBLIN_QWEN_API_KEY', '')
        self.model = model or get_env('CLASSIFY_GOBLIN_QWEN_MODEL', 'Qwen3.8')
...
def _artifact_parts(artifacts):
    ...
    for artifact in artifacts:
        ...
        if kind == 'image':
            ...
            # Verified by the CG-MM-13 local probe (2026-09-21): the endpoint
...
class QwenArtifactBackend:
    ...
    model = 'local-qwen'
    # Modalities this card adds. The CG-MM-01 probe manifest is historical
    # evidence (it verified image); pdf and code are the new capabilities, and
    # video is verified by the CG-MM-13 local probe (2026-09-21: video_url

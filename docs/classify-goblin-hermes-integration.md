# classify-goblin typed advisory layer for Hermes

The loopback endpoint is `POST http://127.0.0.1:8093/v1/systemone`. Requests contain `model`, a JSON `state`, and the typed `questions` returned by `classify_goblin.taxonomy.questions()`. The catalog covers model routing, confidence-gated actions, tool screening, progress, completion, skill selection, compaction, citation verification, RAG filtering, semantic find, composite scoring, and intent routing.

`classify_goblin.workflow.decide(state, client)` sends only bounded routing metadata and returns `advisory.authoritative=false`. The deterministic guard and route are authoritative and cannot be overridden by probabilities.
